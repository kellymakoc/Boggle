...
public class GamePlay4 extends JFrame implements ActionListener{
    ...
    static String[] names = new String[3];// String array for all players' names
    static int[] score = new int[3]; // integer array for all players' score
    static int option; // integer variable for players' level of difficulty
    static String Ailevel;// integer variable for players' level of difficulty
    static int round; // counter for round (Ai getWord() usage)
    ...
    JComboBox <String> aiDifficulty; // scroll down box for players to choose the difficulty of Ai choosing word


    public GamePlay4() throws IOException {
        start = Calendar.getInstance().getTimeInMillis();// get start time
        readFile();// call readFile method
        //set frame
        setTitle("Two Players vs Computer");
        setSize(1000, 600);
    ...
    public void setupLevels(){
        ...
        if (option == 3){ //custom
            ...
            if (result == JOptionPane.OK_OPTION){ // if user press 'ok' button
                ...
                Ailevel = (String) aiDifficulty.getSelectedItem(); // get user choice from scroll down box
            }
        }
        else if (option == 2){ //hard
            Ailevel = "Hard"; // set Ailevel to hard
            ...
            Ailevel = "Medium"; // set Ailevel to medium
            ...
            Ailevel = "Easy";// set Ailevel to easy
    ...
    public void setupConfigPanel(){
        ...
        String [] dif = {"Easy", "Medium", "Hard"}; // store for difficulty
        aiDifficulty = new JComboBox<>(dif);// set a scroll down box
        ...
        config.add(new JLabel("Computer difficulty: "));
        config.add(aiDifficulty);
    }
    ...
    public void setIntroPanel() {
        ...
        welcome = new JLabel("Welcome to Boggle, " + names[1] + " and " + names[2]+ "!");
    ...
    public void setupPlayers() {
        ...
        score1 = new JLabel("'s score: " + score[1]);
        player2_Name = new JLabel("");
        score2 = new JLabel("'s score: " + score[2]);
        Ai = new JLabel("AI");
        scoreAi = new JLabel("'s score: " + score[0]);
        ...
        ImageIcon clouds = new ImageIcon("/Users/kellymak/Downloads/clouds_icon.png"); // import icon for showOption Dialog
        clouds = new ImageIcon(clouds.getImage().getScaledInstance(50,50,Image.SCALE_SMOOTH));
        ...
        names[1] = name1;
        // get Player 2's name
        String name2 = (String) JOptionPane.showInputDialog(null,"Player 2, please enter your name:","Enter Name",JOptionPane.PLAIN_MESSAGE,clouds, null, null);
    ...
    public void setupButtonPanel() {
        ...
        prompt = new JLabel("AI's turn. Please press enter to continue.");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        showUser.setText("");
        String word;
        if (turn == 0) { // if it is Ai's turn
            word = getWord(); // get random word from allValidWords list
            if (!usedWords.contains(word)) { // if the word has not been found
                usedWords.add(word); // add to "used_word" list
                score[turn] += word.length(); // add points to total score
                showUser.setText("AI entered: " + word); // set text to tell user it is a valid word
                // update used_word list on gui
                listModel.clear();
                ...
                showUser.setText(word + " has been found."); // prompt user the word is found
            }
        } else { // player's turn
            showUser.setText("");
            word = userInput.getText().toUpperCase(); // get user input and set to all upper case
        ...
        //switch turns
        if (turn == 2){ // if turn is 2
            turn = 0; // go back to player 1
        }
        else{ // move on to the next player
            turn++;
        }
        if (turn == 0){ // if Ai's turn
            prompt.setText("AI's turn. Please press enter to continue.");
        }
        else{ // if player's turn
            prompt.setText(names[turn] + "'s turn. Please enter a word:");
        }
        // prompt user scores
        scoreAi.setText("Score: " + score[0]);
        score1.setText("Score: " + score[1]);
        score2.setText("Score: " + score[2]);
        userInput.setText("");
    }
    ...
    public static void getAllValidWords(){
        ...
        allValidWords.removeIf(word -> word.length() < MIN_LENGTH);
    }
    ...
    public static String getWord(){
        ...
        if (Ailevel.equals("Hard")){
            return allValidWords.get(allValidWords.size() - round);
        }
        else if (Ailevel.equals("Medium")){
            return allValidWords.get((int) (Math.random() * allValidWords.size()));
        }
        else{
            return allValidWords.get(round);
        }
    }
}